...
// Fixed interest rate values (columns) - 5 below, current, 5 above
const interestValues = computed(() => {
    const base = props.fixedEffectiveRate;
...
function calculateMonthlyHousingCost(
    ...
    // Year 1 interest (using fixed rate parameter)
    const yearlyFixedInterest = fixedLoanPlusBond * (fixedRate / 100);
...
function isCurrentCell(propertyValue: number, interest: number): boolean {
    return (
        ...
        Math.abs(interest - props.fixedEffectiveRate) < 0.01
    );
}
    ...
    <div class="overflow-x-auto">
        <div class="mb-4 flex flex-wrap items-center gap-6">
            <div class="flex items-center gap-2">
                <label
                    for="priceStepFixedInterest"
                    class="text-sm font-medium text-muted-foreground"
                >
                    Price step:
                </label>
                <CurrencyInput
                    id="priceStepFixedInterest"
                    v-model="priceStep"
                    class="h-8 w-32"
                />
            </div>
            <div class="flex items-center gap-2">
                <label
                    for="fixedInterestStep"
                    class="text-sm font-medium text-muted-foreground"
                >
                    Interest step:
                </label>
                <input
                    id="fixedInterestStep"
